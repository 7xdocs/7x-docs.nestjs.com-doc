...
> **警告** 在本文中，你将学习如何基于 **Mongoose** 包使用自定义组件从零开始创建一个 `DatabaseModule`。因此，这个解决方案包含了许多额外的步骤，而你可以使用现成的、开箱即用的专用 `@nestjs/mongoose` 包来省略这些步骤。了解更多信息，请参阅[这里](/techniques/mongodb)。
...
在 Mongoose 中，一切都源自 [Schema](https://mongoosejs.com/docs/guide.html)。让我们定义 `CatSchema`：
...
> warning **警告** 在实际应用程序中，你应该避免使用**魔法字符串**。`CAT_MODEL` 和 `DATABASE_CONNECTION` 都应该保存在单独的 `constants.ts` 文件中。
...
数据库连接是**异步的**，但 Nest 使这个过程对最终用户完全不可见。`CatModel` 类等待数据库连接，而 `CatsService` 则延迟到模型准备就绪才使用。整个应用程序可以在每个类实例化时启动。
...
一个可工作的示例可以在[这里](https://github.com/nestjs/nest/tree/master/sample/14-mongoose-base)找到。
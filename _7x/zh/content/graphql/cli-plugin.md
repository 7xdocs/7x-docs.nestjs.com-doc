...
TypeScript 的元数据反射系统有一些限制，例如无法确定类由哪些属性组成，或者无法识别给定属性是可选的还是必需的。然而，其中一些限制可以在编译时解决。Nest 提供了一个插件，用于增强 TypeScript 的编译过程，以减少所需的样板代码量。

> info **提示** 该插件是**可选的**。如果你愿意，可以手动声明所有装饰器，或者仅在需要的地方声明特定装饰器。
...
根据目前所学，你必须重复大量代码才能让包知道你的类型在 GraphQL 中应如何声明。例如，你可以定义一个简单的 `Author` 类，如下所示：
...
对于中型项目来说，这不是一个大问题，但一旦你拥有大量类，它就会变得冗长且难以维护。
...
该插件基于**抽象语法树（AST）**动态添加适当的装饰器。因此，你将不必在代码中到处使用 `@Field` 装饰器。

> info **提示** 插件会自动生成所有缺失的 GraphQL 属性，但如果你需要覆盖它们，只需通过 `@Field()` 显式设置即可。

#### 注释内省

启用注释内省功能后，CLI 插件将基于注释为字段生成描述。
...
要启用该插件，请打开 `nest-cli.json`（如果你使用 [Nest CLI](/cli/overview)）并添加以下 `plugins` 配置：
...
<table>
  ...
  <tr>
    <td><code>introspectComments</code></td>
      ...
      <td>如果设置为 true，插件将基于注释为属性生成描述</td>
  </tr>
</table>
...
对于标准设置（非 monorepo），要使用带有 SWC 构建器的 CLI 插件，你需要启用类型检查，如[此处](/recipes/swc#type-checking)所述。
...
有了这个文件后，在你的 `jest` 配置文件中导入 AST 转换器。默认情况下（在启动应用程序中），端到端测试配置文件位于 `test` 文件夹下，名为 `jest-e2e.json`。
